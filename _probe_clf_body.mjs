...
export async function run({ THREE, scene, LM, K, errs, args }) {
  ...
  const shade = (p, n, alb, vc, emi, basic) => {
    ...
    for (const d of dirs) E += d.i * Math.max(0, n[0] * d.l[0] + n[1] * d.l[1] + n[2] * d.l[2])
    for (const q of pts) { const dx = q.p[0] - p[0], dy = q.p[1] - p[1], dz = q.p[2] - p[2], dd = Math.hypot(dx, dy, dz); if (q.dist > 0 && dd > q.dist) continue
      E += q.i * LM.distAtt(dd, q.dist, q.decay) * Math.max(0, (n[0] * dx + n[1] * dy + n[2] * dz) / dd) }
    return LM.displayLum(alb * vc * E + emi)
  }
