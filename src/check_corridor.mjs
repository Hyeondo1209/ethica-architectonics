...
console.log('— O. ★셀라(㊶) — 배경 상자 봉인 · 근호 노출 차단 (다섯만 남는다) —')
{
  ...
  }
  {
    ...
    //  ⚠★134 재점검(2026.08.22 · 측정 세션): 해소 조건의 서술이 불완전했다 —
    //   "새 구조물이 들어서면 두 항이 **자동** 복귀한다"는 **거짓이다.** 이 차폐 모델이 아는 항은
    //   ⑴ 드럼 원통 + 개구(passes2D a) ⑵ 리브 몸통(passes2D b) ⑶ 셀라(cellaBlocks) ⑷ 박스(boxBlocks · 스위치 인지)
    //   **넷뿐이다.** 새 구조물은 어느 항에도 안 들어가므로, 실물이 광선을 막아도 이 검사는 계속 붉다.
    //   ⇒ 해소 = 구조물 **+ 그 구조물의 차폐 항 신설**. 둘째 조각을 빠뜨리면 "구조물을 세웠는데 빚이 안 갚아진다"로 오독한다.
    //   ★현재 규모(실측 2026.08.22): 눈 551곳 중 상한 초과 45곳(8.2%) · |k|≥3 누출 48곳(8.7%) ·
    //    498곳(90.4%)은 정상값 5. 누출 리브 = |k| 6~11뿐(k=3·4·5는 한 번도 안 샌다).
    //    초과 눈 분포 = x56~112 · z±2.5(= 철거된 관의 발자국 그대로). 최악 13(★134 선언 시점 15에서 감소).
    const DEBT134 = !BOX_TUBE_ON ? ' ⚠★134 선언된 빚(박스 관 소등 — 새 구조물 + **그 차폐 항 신설**까지 존치. 자동 복귀 아님)' : ''
