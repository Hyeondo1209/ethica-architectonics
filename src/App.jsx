...
const ROOM_HEIGHT  = ROOM_CEIL_Y - ROOM_FLOOR_Y     // 돔 높이(=apex, 현재 49). 지상 돔
const ROOM_OCULUS  = 0.066                // 오큘러스 반각(반지름≈6=박스 반폭) — 실린더가 이 구멍에 앉아 봉합(리브 시야 차단, 꼭대기서만 빛)
...
const ROOM_STAIR_INSET = 0.36                                  // 나선 반지름 = 벽 반지름 × 이 비율(↓일수록 벽에서 더 떨어짐 — 여유·답답함 완화). 튜닝 노브
const ROOM_STAIR_TURNS = 2.5                                   // 총 회전수(스케치 느낌; 핵심 튜닝 노브)
const ROOM_STAIR_WIDTH = 2.4                                   // 디딤판 폭(반지름 방향 = 걷는 길 폭)
const ROOM_STAIR_TOPR  = 5                                     // 꼭대기 코일 최소 반지름(오큘러스·실린더 안에 들어오게)
const ROOM_LAND_R      = 18                              // 착지 디스크 = 실린더 반지름 = 박스 반폭(지름 12 = 박스 가로폭). 크기감 맞춤
const ROOM_CYL_TOP     = 110                               // 빛 우물 실린더 꼭대기 높이(다리 49 위로 솟음). 튜닝 노브
const ROOM_WELL_RT     = 2.5                           // 빛우물 '꼭대기' 반지름(원뿔대 위). ↓=위 좁음=리브 스포 차단↑. 바닥은 ROOM_LAND_R(6)=디스크와 맞물려 봉합
...
function DefAxiomRoom() {
  ...
  // 나선 치수 — 꼭대기를 다리 높이 '바로 아래'에서 끝낸다(맨 윗 디딤판 윗면 = 다리 윗면). 그 위는 평평한 착지 디스크.
  const TREAD_VERT = ROOM_STAIR_RISE * 3.5                         // 수직 두께(아래 칸과 겹쳐 '속찬' 계단)
  const TOP_Y      = COR_Y0 + COR_THICK / 2 - TREAD_VERT / 2       // 맨 윗 디딤판 윗면이 다리 윗면(=COR_Y0+두께/2)과 일치
  ...
  return (
    <group position={[ROOM_CX, 0, 0]}>
      ...
      <mesh>
        <tubeGeometry args={[stringer, 240, 0.35, 10, false]} />
        <meshStandardMaterial color="#8f6c3e" roughness={0.85} />
      </mesh>
      {/* 꼭대기 착지 디스크 — 나선 끝을 평평히 덮어 다리와 수평 연결 + 오큘러스 봉합 */}
      <mesh position={[0, COR_Y0, 0]} userData={{ walkable: true }}>
        <cylinderGeometry args={[ROOM_LAND_R, ROOM_LAND_R, COR_THICK, 40]} />
        <meshStandardMaterial color="#c2a062" roughness={0.9} side={THREE.DoubleSide} />
      </mesh>
      {/* 솟은 실린더(빛 우물) — 착지 위로 높이 솟는다. 올려다보면 좁은 시야각으로 꼭대기의 빛만
          보이고 둘러싼 벽이 경선 리브를 가린다(스포일러 차단). 꼭대기는 열려 빛이 샤프트로 내려온다 */}
      <mesh position={[0, (ROOM_CEIL_Y - 3 + ROOM_CYL_TOP) / 2, 0]}>
        {/* 원뿔대(frustum): 위=ROOM_WELL_RT(좁음·스포 차단) / 아래=ROOM_LAND_R(넓음·디스크와 봉합·헤드룸) / 위 열림 */}
        <cylinderGeometry args={[ROOM_WELL_RT, ROOM_LAND_R, ROOM_CYL_TOP - (ROOM_CEIL_Y - 3), 40, 1, true]} />
