...
const ROOM_LAND_R      = 18                              // 착지 디스크 = 빛우물(원뿔대) 바닥 반지름. (사용자 튜닝 18 — 박스 반폭과의 일치는 더는 강제 안 함)
const ROOM_DISC_HOLE   = 6                               // 디스크 가운데 구멍(=천장 개방) 반지름 ≈ 오큘러스(6). 나선이 이 구멍으로 올라오고 빛우물이 위로 보임. ↑=구멍 큼
  ...
  return (
    <group position={[ROOM_CX, 0, 0]}>
      ...
      <mesh position={[0, COR_Y0 + COR_THICK / 2, 0]} rotation-x={-Math.PI / 2} userData={{ walkable: true }}>
        <ringGeometry args={[ROOM_DISC_HOLE, ROOM_LAND_R, 48]} />
