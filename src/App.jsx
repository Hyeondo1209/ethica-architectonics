...
const COR_CX       = (COR_X0 + COR_X1) / 2   // 원통 중심 x(=126, 길 중간). 바닥 원은 지면(0)에 앉음
const COR_R        = 84                       // 원통 반지름(126 ± 84 → 방쪽 42 ~ 리브쪽 210 커버)
const CEIL_LO      = 70                        // 천장 낮은 쪽(방, −x) 높이
const CEIL_HI      = 150                       // 천장 높은 쪽(리브, +x) 높이 ← 공간감 핵심 노브
const CEIL_SLOPE   = (CEIL_HI - CEIL_LO) / (COR_X1 - COR_X0)    // 빗면 기울기(x의 함수)
const ceilY        = (x) => CEIL_LO + CEIL_SLOPE * (x - COR_X0)  // 천장 평면 y = LO + slope·(x−42)
const COR_THICK    = 0.6                       // 바닥·박스 판 두께
const COR_WALL_SEG = 96                        // 원통 벽 둘레 분할 수
// +x 벽 창(이웃 ±2 프레이밍)
const WIN_HALF     = Math.PI / 6              // 창 좌우 반각(±30° → z=±42; ±2=36.5 보임/±3=54.4 가림)
...
const BOX_X0       = COR_X0 - 16               // 박스 바깥 끝(방 쪽)
const BOX_X1       = COR_X0 + 20                // 박스 안쪽 끝(원통 안으로 살짝 물림)
...
function Corridor() {
  ...
  // 원통 벽: 바닥(0)→빗면 천장. +x에 창(이웃 ±2), −x(θ=π)에 박스 문(트임).
  const wallGeo = useMemo(() => {
    ...
    for (let i = 0; i < COR_WALL_SEG; i++) {
      ...
      if (dZero <= WIN_HALF) {                                      // +x 창: 가운데(SILL~TOP) 비움
        ...
        quad(xa,0,za, xb,0,zb, xb,yb,zb, xa,ya,za)
      }
    }
  ...
  return (
    <group>
      ...
      {/* === 방쪽 직육면체 박스(짧은 닫힌 터널): 양 측벽 + 천장, 양끝 트임 === */}
      {[1, -1].map((sgn) => (
        <mesh key={'boxw' + sgn} position={[(BOX_X0 + BOX_X1) / 2, BOX_TOP / 2, sgn * BOX_HW]}>
          <boxGeometry args={[BOX_X1 - BOX_X0, BOX_TOP, COR_THICK]} />
          <meshStandardMaterial color={wallMat} roughness={0.9} side={THREE.DoubleSide} />
        </mesh>
      ))}
