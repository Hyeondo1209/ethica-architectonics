...
//     'ramp' 기준선 = **안쪽 코너선 − T**(두께 위계를 지키는 유일한 선 — 근거는 constants ★89 블록)
...
//   'ramp' 규칙: ②쪽은 **높은-방위 끝이 안쪽(뒤) 코너**이고 낮은-방위 끝이 노징이다(방위가 커질수록
//   올라가므로 내려가는 쪽 = 낮은 방위). 안쪽 코너 = `y − T` · 노징 = `y − R − T`
//   ⇒ 두께가 **안쪽 코너에서 T(1.500), 노징에서 T+R(1.917)** — constants ★89 '안쪽 코너선 − T'의 구간 표현.
//   ⚠구 주석은 이 둘을 뒤집어 적었다(2026.07.30 ★91 정정 — 값·구현은 처음부터 맞았고 이름만 틀렸다).
export function terraceRuns () {
  ...
  for (let i = 0; i < s.N; i++) {                                                // ④ (거울 계단)
    const y = TR_Y - (i + 1) * R
    //  거울 쪽은 az0이 |방위| 큰 쪽 = 안쪽 코너 쪽이다(②의 az1과 대칭)
...
//  ── 밑면의 높이(로컬 방위 → y) ─────────────────────────────────────────────
//  'saw'  = 걷는 면 − T (톱니)
//  'ramp' = **안쪽 코너선 − T**. 안쪽 코너선 = 디딤 뒤 코너들을 꿴 직선이고,
