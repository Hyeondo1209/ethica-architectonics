...
function archSpec({ pts, E, xE, hwOut, y, B, o }) {
  ...
  //  ★닫힌형: y(s) = yJ + (yB − yJ)·(1 − √(s/L))²  ← ★133-b의 두 접선 조건을 s 위에서 푼 것
  const yOfS = s => yJ + (yB - yJ) * Math.pow(1 - Math.sqrt(Math.max(0, Math.min(1, s / L))), 2)
  ...
  for (let i = 0; i + 1 < path.length; i++) {
    const d = [path[i + 1][0] - path[i][0], path[i + 1][1] - path[i][1]], l = nrm(d) || 1
    dirs.push([d[0] / l, d[1] / l])
  }
  const kinkCos = Math.cos((o.archKink ?? LK4_ARC_KINK) * Math.PI / 180)
  const cuts = []
  for (let i = 1; i < dirs.length; i++) {
    if (dirs[i - 1][0] * dirs[i][0] + dirs[i - 1][1] * dirs[i][1] < kinkCos) cuts.push(i)
  }
  const runs = []
  let a0 = 0
  ...
  return { on, zEnd, zEndAsked: o.archZ ?? LK4_ARC_Z, zMin, clamped, zCol, yJ, yB, path, sArr, L, yOfS, rise: yJ - yB, hwOut, reach,
    dirs, runs, corners,
...
export function buildLink4Arch(S = link4Spec()) {
  const A = S.arch
  if (!S.on || !A.on) return null
  const hw = A.hwOut, yTop = A.yTop
  const pos = []
  const tri = (a, b, c) => { for (const q of [a, b, c]) for (const v of q) pos.push(v) }
  const quad = (a, b, c, d) => { tri(a, b, c); tri(a, c, d) }
  //  경로 위 임의 점의 호길이 → y (구간 선형 보간으로 s를 얻는다 — y는 닫힌 식)
  const sOfPoint = q => {
  ...
  //  ① 마디마다 스윕(마디 안은 조밀 — 다면체로 보이던 원인 제거)
  for (const [i0, i1] of A.runs) {
    ...
    const F = P.map((p, j) => { const n = nOf(j), yb = A.yOfS(sOfPoint(p)); return { p, n, yb } })
    const cs = f => [[f.p[0] - f.n[0] * hw, f.yb, f.p[1] - f.n[1] * hw], [f.p[0] + f.n[0] * hw, f.yb, f.p[1] + f.n[1] * hw],
                     [f.p[0] + f.n[0] * hw, yTop, f.p[1] + f.n[1] * hw], [f.p[0] - f.n[0] * hw, yTop, f.p[1] - f.n[1] * hw]]
    ...
    quad(a0[0], a0[3], a0[2], a0[1])
    quad(a1[0], a1[1], a1[2], a1[3])
  }
  //  ② 꺾임 채움 — 각기둥(현도 "직육면체로 채워도 돼"). 두 마디 끝면의 볼록껍질을 밑면 높이 하나로 밀어 올린다.
  //   밑면 = 그 자리의 y(s)라 양 마디와 **정확히 같은 높이**에서 만난다(틈 0 · 단차 0).
  for (const c of A.corners) {
    ...
    for (let i = 0; i < N; i++) {
      ...
      quad([a[0], yb, a[1]], [b[0], yb, b[1]], [b[0], yTop, b[1]], [a[0], yTop, a[1]])
    }
    for (let i = 1; i + 1 < N; i++) {
      tri([H[0][0], yTop, H[0][1]], [H[i][0], yTop, H[i][1]], [H[i + 1][0], yTop, H[i + 1][1]])
