...
export const CL_ROOF  = 20.0                  // 회랑 내부고
...
export const LAMP_ENTRY_Y = 263.4                  // 관이 리브 표면(중심선 거리 6.0)을 뚫는 높이 — 도출: check_lamps.mjs 스캔.
                                                   //   지붕 상면(≈257.03) 위 노출 ≈6.4 = 테라스에서 보이는 '리브에 꽂힌 관들'
export const LAMP_TOP_Y   = LAMP_ENTRY_Y + 2.5     // 관 상단 캡(보어 내부 — 불가시). 진입 후 잔류는 check_lamps.mjs가 검증
export const LAMP_MOUTH_Y = 8.0                    // 갓 입(아래끝)의 바닥 위 높이 — 보행 헤드룸(눈 1.6) 노브
export const LAMP_FUNNEL_H = 0.5                   // 갓 높이(입→목)
export const LAMP_MOUTH_R  = 1.55                  // 갓 입 반지름(목 반지름 = LAMP_TUBE_R)
...
export const RAD_PRY     = 10.5               // ★꽃잎 수직 반축(8.5→10.5). 셸 41.0~62.0(유선형 렌즈 유지)
export const RAD_PCY     = 51.5               // ★꽃잎 중심고(50→51.5). 돔 최소 간극 2.96 · 실내 바닥 위 12.2 · 바닥 유효반경 15.6(전부 검증치)
