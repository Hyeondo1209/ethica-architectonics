...
// ── 무릎길: 나선 끝(U_SPIRAL_END) → 무릎 반대편 대칭점. 대칭 유도(수치의 정본):
//    경사(u) = 0.32·cosh²((u−KNEE)/WIDTH) 이 짝함수 → 임계 0.6의 두 해가 KNEE 대칭.
//    ③에서 u 0.2333→0.2667 · y 224→256 · x 269→187 · 92칸(경사 0.6→0.32→0.6, 계단이라 가변 경사 흡수) ──
export const U_KNEE_END   = KNEE + (KNEE - U_SPIRAL_END)                    // 대칭 = +WIDTH·acosh(√(0.6/0.32))
export const KW_CLIMB     = (U_KNEE_END - U_SPIRAL_END) * H                 // ③에서 ≈32.1
export const KW_STEPS     = Math.max(8, Math.round(KW_CLIMB / STEP_RISE))   // ③에서 92
export const KW_TREAD_W   = 4.5            // 무릎길 디딤판 z폭(관내 현폭 12의 중앙 보도). 깊이 = TREAD_DEPTH 공유
//  디딤판 = 중심선 높이 '부양'(바닥 강하 0) — 나선 디딤판과 같은 문법(1p7 '증명된 뜸'의 연속). 위치 = (rOf(y/H), y, 0)

// ── 착지 디스크(공용): 나선끝·갈림 2곳. r=4.0 > 헬릭스 반지름 3.3 → 나선 마지막 판을 각도 무관하게 받음
//    (θ_end가 칸수 튜닝에 흔들려도 안전 — f축 원칙과 동형) ──
...
export const X_DESC0      = rOf(U_KNEE_END) - LAND_R + 1.2                  // 하강 시작 x — 디스크 원 '안'으로 1.2 물림(첫 판 도달 보장) ③≈184.2
