...
// 통로 상자(z중심 = JCT_DN_Z, 내부 반폭 PASS_HW): 바닥 슬랩 · 볼벽 · 지붕 · 린텔 · 끝벽(문 = 1p11 공개점)
export const PASS_HW      = 2.3            // 내부 반폭(폭 4.6 — 전실은 좁게)
export const PASS_T       = COR_THICK      // 판 두께 0.6(통로 외피 어휘 공유)
export const PASS_ROOF_H  = 5.5            // 전실 내부고(압축!) — 지붕 윗면 ③≈254.1 < 리브 하부면(검증 스크립트)
...
export const PASS_X_END   = TERRACE_ROUT + 0.5                              // 끝벽 x(테라스 물림) ③≈158.9
// 린텔(폐쇄 패널): 웨지(아치 입 구간)와 전실 사이 상인방 — 입 상단 너머 시선(지붕 위로 넘어가는 유일 누출로)을 봉쇄.
//  하단은 보행자 머리(바닥 도달 시 ≈251.6) 위: 밑을 지나며 '압축의 문턱'이 자연 발생(2절).
export const LINTEL_X     = X_DESC_END - 0.1                                // 패널 x(두께 PASS_T) ③≈175.9
export const LINTEL_Y0    = PASS_FLOOR_Y + 5.2                              // 개구 높이 5.2(머리 여유 ≥1.4)
export const LINTEL_Y1    = U_KNEE_END * H + 1.2                            // 상단 — 입 상단(아치 컷 윗변)보다 1.0 위
...
// ════════ 연결 통로 본체 재구성 (1p9 증가 → 1p10 독립 → 문 압축) — ★신규 기하 2026.07.07 ════════
//  진행 = +x(하강 끝 X_DESC_END) → −x(문 PASS_X_END). '하나에서 여럿으로'(게루 9·10 이행)의 공간화:
//   · 1p9 = 폭·천장이 계단식으로 '더해짐'(실재성↑→속성↑)
//   · 1p10 = 넓은 방 + 흩어진 독립 벽감(각 속성은 자기 통해 파악) ⚠수직 립 배열 금지(리브 전용 서명 §2-C)
//   · 압축 = 문으로 폭·천장 수축 → 문 밖 무한 리브(여럿→무한)
//  ⚠ 통로 본체는 리브 −x면 밖(X_DESC_END 177.8 < 리브 −x면 ≈181)이라 폭·천장 자유 — 그 x에서 리브는
//    y≈900+에 있어 무충돌(검증 스크립트로 실측). 밀폐(스포 3중 ③)는 볼벽·지붕이 폭 따라 넓어져 유지.
export const P9_STEPS  = 3                    // 1p9 증가 단 수
export const P9_LEN    = 7.8                  // 1p9 x 길이(하강 끝에서 −x)
export const P9_HW0    = PASS_HW              // 시작 반폭(하강 폭 승계 2.3)
export const P9_HW1    = 5.0                  // 끝 반폭(넓어짐)
export const P9_ROOF0  = PASS_ROOF_H          // 시작 천장고 5.5
export const P9_ROOF1  = 8.5                  // 끝 천장고(상승)
export const P9_X0     = X_DESC_END           // 시작 x = 하강 끝(파생·불변)
export const P9_X1     = P9_X0 - P9_LEN       // 1p9→1p10 경계
export const P10_LEN   = 7.0                  // 1p10 방 x 길이
export const P10_HW    = 5.5                  // 1p10 방 반폭
export const P10_ROOF  = 9.0                  // 1p10 천장고
export const P10_X1    = P9_X1 - P10_LEN      // 1p10→압축 경계
export const NICHE_D   = 1.6                  // 벽감 깊이(z 바깥 오목)
export const NICHE_H   = 3.4                  // 벽감 높이(바닥에서)
//  벽감 3개(흩어진 독립 자리 — 배열 아님): [x중심, side(−1=−z벽 / +1=+z벽), 폭 w]
export const NICHES    = [ [P9_X1 - 1.8, -1, 2.2], [P9_X1 - 3.6, +1, 2.0], [P9_X1 - 5.4, -1, 2.4] ]
export const PC_STEPS  = 4                    // 압축(문으로 수축) 단 수  (문목 최종 = PASS_DOOR_W/2·PASS_DOOR_H)
