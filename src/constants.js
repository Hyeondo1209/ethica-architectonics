...
// +x 벽 창 — ★1p5 재재설계 ㊳(2026.07.14): 구 파노라마 ±43° '복원'. ㊱ 슬릿('하나의 프레임')은
//  ⚠의도적 폐기(편차 아님) — 1p5는 존재 명제가 아니라 불가능성 명제(non potest dari): 여럿을 세워 놓고
//  무너뜨려야 한다. ㊱의 폐기 근거 2건의 재답변: ①공개 곡선 역전 → 재정의(1p5 = '부정된 다섯' vs 1p9 =
//  '긍정된 넷' — 질이 다르므로 역전 아님, 현도 승인) ②열주 오독 → 불규칙 문 높이 + 뻗는 계단이 콜로네이드
//  질서를 깬다 + 신전 파사드는 이제 의도(TEMPLE_MODE — ㊴ 프리즈 확장). ★±43°가 정확히 리브 #±2(드럼각 32.1°)까지
//  담고 #±3(45.1°)을 자른다 — '다섯'은 기하가 정한 숫자. 검증 = check_corridor.mjs C절(가시성).
export const WIN_HALF   = 43 * Math.PI / 180   // ★창 반각 — 다섯의 프레임(±2 포함·±3 배제의 기하 근거)
export const WIN_SILL_Y = 0            // 아래턱 0 = 리브를 발끝(지면)까지
export const WIN_TOP_Y  = CEIL_HI      // 위턱 = 천장(천장이 낮은 구간은 개구가 천장까지)
...
// ── ★플랫폼 강하(㊴ 2026.07.17, 현도 소견 2): 다리(수평 49.3) → 하강 계단 → 낮은 플랫폼 ──
//  "플랫폼을 조금 내려다보는 것이 숭고한 분위기" — 진입 시퀀스 = 수평 → 하강 → 결절 → 다섯 계단.
//  ⚠불변식 역전: RIB_Y=74가 정본(방·통로 동결의 귀결, 문 높이 불변)이고 COR_CLIMB이 파생으로 바뀜.
export const PLAT_DROP    = 18             // ★㊴-5 플랫폼 강하 깊이(현도: "플랫폼이 더 아래에, 하강 계단이 더 많이") — 구 3.5. 노브
// ★㊴-5(현도 확정 그림): **짧은 수평 다리(박스 출구 +5에서 끝) → 긴 하강 계단(≈51m·42단) → 깊은 제단(결절)**.
//  DESC_X0 = 박스 출구 +5가 정본(다리가 입구 근처에서 끝나는 것이 불변) · DESC_X1 = 플랫폼 서쪽 림(착지) —
//  하강 '길이'는 둘의 차로 자동. 정의는 순서 제약상 PLAT_X 직후(파일 하단). (구 DESC_LEN 노브 폐기 — 파생으로 역전)
export const PLAT_Y       = COR_Y0 - PLAT_DROP   // 플랫폼 판 중심 y = 45.5
export const RIB_Y        = 74             // ★리브 접합(문) 높이 — 불변식 그 자체(구 PLAT_Y+CLIMB 파생을 역전)
export const COR_CLIMB    = RIB_Y - PLAT_Y // #0 총 상승 = 28.5(파생). ⚠구 COR_STEPS(직선 계단 칸수)는 ㊳으로 폐기
...
// ── ★신전 부재(㊳ 대들보 → ㊴ 프리즈 확장 2026.07.17) — "파르테논 같은 신전 입구" ──
//  ★㊴ 현도 소견 3: "대들보 위에 경선리브들이 드러남 — 드러나는 부분이 있으면 안 됨(신전 느낌이 안 남)."
//   해법 = 대들보를 **창 위턱(빗면 천장)까지 채움**: 하단 TEMPLE_Y0 위 창 상부 전체가 수평 부재 =
//   문 위 전체가 프리즈인 진짜 신전 파사드. ★리브는 자르지 않는다 — 부재가 '가릴' 뿐(LOCKED 무결).
//   리브 다섯은 물리적으로 관통(CSG 구멍 유지 — 구멍은 부재 안이라 안 보임). 관통해 위로 계속 가는
//   사실은 불변("우리가 신전을 지어도 실체는 그 위로 간다"), 이제 진술은 1p11 테라스에서 회수.
//  ⚠구 3모드(㊳)는 ㊴로 폐기·정리: 'entablature'(리브 클립 = LOCKED 의식적 위반 장치)는 가림 해법이
//   그 존재 이유를 소멸시켜 **제거**(위반 장치가 불필요해짐 — 좋은 소멸). 'frame'(약한 신전감)은 소견 3이
//   사실상 기각. 남은 스위치 = 'beam'(기본) / 'off'.
//  ★기하(㊴): 창 상부를 '각도로도' 봉인해야 함(구 D=18은 창 모서리 부근 y114+에 배경 비침 틈) →
//   앞면을 창 평면(x≈265.4 = 드럼각 43° 지점)까지 당기고 z반길이를 창 전폭+로. 상면 = 빗면 천장 정합
//   (ceilY(x) − 0.02, x방향 경사 상면 — 천장 위 돌출 없음 = 회랑·테라스 조감 무오염). 검증 = C절 상부 봉인.
export const TEMPLE_MODE = 'beam'
export const TEMPLE_Y0   = 114              // ★부재 하단 y — 최고 문 상단(110) 위 4. 창 y114~천장 = 전부 프리즈
export const TEMPLE_X0   = COR_CX + COR_R * Math.cos(43 * Math.PI / 180) - 1.5   // 앞면 ≈263.9 = 창 모서리 x − 1.5(모서리 스침 여유)
export const TEMPLE_X1   = 295              // 뒷면 — 리브 관 단면(x≤294) 포함 여유 1
export const TEMPLE_HZ   = 62               // z반길이 — 창 전폭(±57.3) + 리브 #±2 관 여유
export const TEMPLE_CLR  = 0.4              // 관통 구멍 반경 여유(리브 표면과 z파이팅 방지)
// ── 길 ──
...
//    (⚠구 근거였던 '광학 밀폐'는 ㊳ 창 복원으로 실효 — 상한 35°는 이제 #0 곡선 계단의 프로파일이 지므로 f의 실질 상한도 완화.
//     단 f를 크게 옮기면 STAIR5 곡선(특히 #0 끝 접근·±2 간극)의 재판정 필요 — check_corridor가 자동 재검).
//    하한 0.10 = 원판이 박스 옆벽을 뚫는 지점. 검사 = check_corridor G절 5항.
export const PLAT_F = 0.4                           // ★위치 노브(㊴-4·㊴-5 현도: 수평 위치는 원판 유지). 0.4 → x187.2
export const PLAT_X = COR_CYL_X0 + PLAT_F * (COR_X1 - COR_CYL_X0)   // f=0.149 → 145.0
export const DESC_X1 = PLAT_X - PLAT_R + 1.0        // 하강 끝 = 플랫폼 서쪽 림 +1 물림(f=0.4에서 ≈180.2) — 착지
export const DESC_X0 = BOX_X1 + 5                   // ★㊴-5 하강 시작 = 박스 출구 +5(=129) — 다리는 입구 직후에 끝난다(정본)
...
export const RIB_TINT_AMT  = 0.00 //0.15             // 알베도 혼합 최대치(정점에서)
export const RIB_TINT_EMIS = 0.00 //0.10            // 발광 성분(안개 관통 보조)
