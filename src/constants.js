...
export const MIR_PADS = [
  { id: 'drum-hall', cx: COR_CX, cz: 0, r: 128, t: 2.0 },
]
...
//  ★★구 띠 깊이는 **노브가 아니라 파생**이다: 넓은 쪽 끝을 구의 적도에 두면 R_s = max(r0,r1)이고
//   깊이 = √(R_s² − min(r0,r1)²)로 떨어진다. 부산물이 크다 — **적도에서 기울기가 수직**이라 구 띠가
//   이웃 원기둥에 **접선으로** 만난다(그 이음매만은 반경뿐 아니라 기울기까지 이어져 완전히 매끄럽다).
...
//  구간: t = 'cyl'(등반경) · 'cone'(선형) · 'sph'(구 띠 — 깊이 파생). r = 그 구간이 **끝나는** 반경.
//  d = 깊이(cyl·cone만. sph는 파생). 첫 구간의 시작 반경 = 원기둥 반경 16.25(연속).
export const RAD_CYL_TERM = {
  spike: [ { t: 'cone', r: 1.8, d: 30 }, { t: 'cyl', r: 1.8, d: 36 } ],                       // ① 원뿔대 · 침
  drum:  [ { t: 'sph', r: RAD_CYL_MASS_DRUM }, { t: 'cyl', r: RAD_CYL_MASS_DRUM, d: 22 },     // ② 구 · 원기둥
           { t: 'sph', r: 7.0 }, { t: 'cyl', r: 7.0, d: 16 }, { t: 'cone', r: 3.2, d: 26 } ], //   구 · 목 · 원뿔대
  cone:  [ { t: 'cone', r: 3.5, d: 52 } ],                                                    // ③ 긴 원뿔대 하나
  orb:   [ { t: 'sph', r: RAD_CYL_MASS_ORB }, { t: 'cyl', r: RAD_CYL_MASS_ORB, d: 12 },       // ④ 구 · 원기둥
           { t: 'sph', r: 4.5 }, { t: 'cone', r: 1.8, d: 10 }, { t: 'cyl', r: 1.8, d: 40 } ], //   구 · 원뿔대 · 침
}
...
export function termSpec(name) {
  ...
  for (const g of segs) {
    const d = g.t === 'sph'
      ? Math.sqrt(Math.max(r, g.r) ** 2 - Math.min(r, g.r) ** 2)   // ★파생(적도 접선)
      : g.d
...
export const RAD_CYL_SPH_SEG = 12         // 구 띠 세로 분할(스윕 매끄러움 — 12면 최대 각오차 < 4°)
