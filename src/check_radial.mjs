...
console.log('── 11. 컷 바닥·바닥 원판(2026.07.11 ②c·확장) ──')
{
  ...
  const shellRAt = (y) => RAD_PRX * Math.sqrt(Math.max(0, 1 - ((y - RAD_PCY) / RAD_PRY) ** 2))
  const FLOOR_R = shellRAt(COR_Y0 - COR_THICK / 2) - 0.1
  ok('바닥 원판이 셸 안(밑면 모서리 기준)', FLOOR_R + 0.05 < shellRAt(COR_Y0 - COR_THICK / 2), `원판 r=${r2(FLOOR_R)}`)
  ok('바닥 원판 ≥ 터널 끝 겹침 1.5(길 연속)', FLOOR_R > (RAD_PRX - 2.5) + 1.5, `${r2(FLOOR_R)} > ${r2(RAD_PRX - 1)}`)
}
...
console.log('── 14. 문틀 마감(2026.07.11) ──')
{
  ...
  const backNeed = Math.sqrt(Math.max(0.25, sR(LIN_TOP) ** 2 - FR_OUT ** 2))
  const FR_BACK = backNeed - 0.25, FR_FRONT = sR(RAD_PCY) + 0.25
  const FR_C = (FR_FRONT + FR_BACK) / 2, FR_D = FR_FRONT - FR_BACK
  const TUBE_END = FR_BACK + 0.2
  ok('문틀이 셸 표면을 걸침(뒤 0.25·앞 0.25 여유)', FR_BACK < backNeed - 0.2 + 1e-9 && FR_FRONT > sR(RAD_PCY) + 0.2 - 1e-9,
    `뒤 ${r2(FR_BACK)} < ${r2(backNeed)} · 앞 ${r2(FR_FRONT)} > ${r2(sR(RAD_PCY))}`)
...
console.log('── 15. 모듈 평가 스모크(런타임 상수 오류 — TDZ 등) ──')
{
  ...
  try {
    ...
    writeFileSync(join(dir, 'run.mjs'), `import('./rad.mjs').then(() => process.exit(0)).catch(e => { console.error(e.message); process.exit(1) })`)
    execSync(`node ${join(dir, 'run.mjs')}`, { stdio: 'pipe' })
  } catch (e) {
    evalOk = false; msg = (e.stderr || e.stdout || '').toString().split('\n')[0]
  }
  ok('Radial.jsx 번들 평가(임포트) 성공', evalOk, msg)
}
