...
console.log('\n— W. 보행 (FREE_WALK를 끄면 걸어서 완주할 수 있는가) —')
{
  ...
  console.log('  · W4. walkable 태그 선언 대장 (JSX 정적 대조)')
  {
    ...
    const LEDGER = [
      ['Corridor.jsx',      42, 17, 0, 9],   // ★★★215(09.06) +3 = 프리즈 방 빛 볼륨(아가리 빛기둥·그 후광·그루터기 후광 — 전부 walkable false). 39→42 · false 6→9   // ★★★214(09.06) +1 = 갓 치마 커튼 가닥 메시 · ★214-l +1 후광 통(둘 다 walkable false — 빛). 37→39 · false 4→6   // ★★★147-g(08.19) +1 = 갓 서쪽 처마 살(밟는 면 아님 — walkable 태그 없음. 36→37) · ★94-c 중앙 기둥 · ★95 반십각 기둥 · ★96 헌치 · ★98 서쪽 빗면(전부 walkable:false)
      ['Dome.jsx',          79, 21, 4, 6],   // ★★★240(09.24 셋째) +1 = 입구 층계참 양 끝면(lde — map 한 줄 · 무선언) 78→79 · ★★★239-d(09.24 셋째) +1 = 등불 방 기둥 빛 안개(LampGlow — 무선언 · 볼륨) 77→78 · ★★★225(09.24) +1 = 등불 빛기둥(LampBeam — 무선언 · 회랑 9기·등불 방이 같은 컴포넌트) 76→77 · ★★★224(09.20) +1 = 등불 리브 9기 instancedMesh(LampRibs — 무선언 · 갓 입 발광면 <mesh는 조건부 게이트로 리터럴 존치) 75→76 · ★★★223(09.18) +1 = 회랑 **시작 끝캡**(φ0 · 벽·무선언) 74→75 · ★221-b(09.18) ±0 = 등불 방 뿌리 목은 <LampRoot/> 재사용(대장은 <mesh 리터럴 수) · ★★★221(09.14) +1 = 등불 뿌리 목(LampRoot — 무선언) 73→74 · ★★★220(09.13) +1 = 회랑 입 아치 패널(CloisterMouthWall — 벽·무선언). 72→73   // ★87 +1 = MirrorPads · ★90 +1 = 리드 연결 계단 · ★93 +1 = 하판 고리판(전부 walkable)
      ...
      //  구 Tunnel 3메시도 보존계로 남아 함께 세어진다. 체제를 바꿔도 이 수는 불변.
                        ['Radial.jsx',        29, 11, 0,  9],  // ⛔★142(08.17) −1 = 두 번째 팔 **삭제**(★140분 회수 — 소등이 아니라 마운트 자체가 사라졌다) · ★139(08.15) +1 = 갈래 팔(false — 밟는 면 아님) · ★130(08.14) +2 = 접속 통로 밀봉 관·나선(walkable) + 미니 첨탑(false) · ★91/119/121/122 계열 · ★122-Q 문지방 · ★122-R 어귀 판(walkable:true) · ★123 +1 = 적도 칼라(false) · ★126(08.13) +1 = 1p3 지지 팔(false — 밟는 면 아님. 센서스는 소스 기준 — k 게이트 무관)
      ['RadialEvents.jsx',   6,  1, 0, 0],
      ['Room.jsx',          61, 20, 0, 31],   // ★★★213-b(09.05) +1 = 샤프트 빛 슬라이스 메시 분리(세기 별도 · false). 60→61 · false 30→31.   // ★★★213(09.04) +1 = 월대샤프트 빛 볼륨 메시(walkable false — 빛). 59→60 · false 29→30.   // ★★★212-h(09.04) +1 = 문 빛 장치 메시(무선언·빛). 58→59.   // ★★★211-m(09.04) +1 = 관 서단 스킨(walkable false). 57→58 · false 28→29.   // ★★★211-i(09.03) +1 = 횡단면 슬라이스 메시(무선언·빛). 56→57 · ★★★211(08.30) +1 = 관 빛 커튼 map 한 자리(무선언 — 빛 메시, 밟는 면 아님. BRD_LIGHT_ON 게이트는 useMemo 빈 배열이라 센서스는 소스 기준 유지). 55→56 · ★★★175-f(08.25) −1 = 빛기둥 4사슬(상절·하절·헤일로 상·하)을 **map 한 자리**로 합침(구 core/halo 두 자리 → 하나). 56→55 · ★★★169(08.22) +1 = 방 껍질 솔리드(false — 벽. 구 종잇장 2장은 보존계 분기에 남아 함께 세어진다). 55→56, false 27→28 · ★★★154(08.21) +1 = 첨탑 az0° 문틀. 54→55   // ★★★150(08.20) +1 = 사다리꼴 관 solid map(false). 53→54 · ★★★148(08.20) +1 = 관 볼트 solid(웹·횡단아치·대각리브·벽앞기둥·첨탑대역 — map 한 자리, false). 52→53, false 24→25 · ★★★147(08.19) +2 = 접속 통로 walk(데크판·직각나선·직선 계단 — 소스상 map 한 자리, walkable) + solid(측벽·지붕판·기둥 — map 한 자리, false). 50→52, walk 19→20, false 23→24 · ★★★146-b(08.18) 접합부 개작: 접합판+아치 기둥 **2메시**(-a의 기둥머리 소멸 — 목이 기둥에 통합. 51→50, false 24→23) · ★145-e(08.18) 안벽 = 상인방+열주 — solid map 한 자리 그대로(파츠 배열만 늘었다) · ★★145-d(08.18) 고리 통로 1메시 → 회랑 파츠 2메시(walk map = 바닥판·지붕판 상면 보행 / solid map = 안벽·아케이드·난간) · ★★★145(08.18) +4 = 돔 리브 + 통로 기둥(false — 소스상 회전 마운트 map 안 둘) · 띠(false) · ★★★★144-b(08.17) +2 = 내벽 나선 본체(walkable — 밟는 면) + 난간 턱(false — 밟는 면 아님. 소스상 map 한 자리씩 둘) · ★★★143(08.17) +1 = 1p2 통로 기둥+아치①(false — 밟는 면 아님. 소스상 map 한 자리) · ★137(08.15) +2 = 1p3 통로 walk(관2·참 — map 한 자리) + solid(기둥·아치 — map 한 자리, false) · ★136-c(08.15) +1 = 접합부 아치(false — 밟는 면 아님) · ★136(08.15) +1 = 1p4 접속 관(walkable — 걷는 관. 소스상 map 한 자리) · ★133(08.15) +2 = 복합체 walk(계단 매스 2·참 — 소스상 map 한 자리) + solid(벽·지붕·캡·기둥·아치·포털 — map 한 자리, false) · ★131(08.14) +1 = 새 층 플랫폼+계단 2기(walkable — 셋 다 밟는 면. 소스상 map 한 자리) · ★128(08.14) +1 = 첨탑 테라스(walkable — 사람이 서는 고리 판. 센서스는 소스 기준이라 SPT_ON 게이트와 무관) · ★117(08.05) +1 = 감실 처마 8기(false — 밟는 면 아님) · ★116(08.05) +1 = 방 돔 살 8기 — ⛔현도 반려로 RRIB_ON=false(보존계). 마운트 자체는 남아 있어 인벤토리 수는 유지 · ★115(08.05) +1 = 방 돔 살 8기(false — 밟는 면 아님) · ★115(08.05) +1 = 뿌리 십자 마구리(false — 밟는 면 아님) · ★114(08.05) +1 = 벽 밑동 팔각 각뿔대(false — 밟는 면 아님) · ★111(08.04) +1 = 공리 볼트(문) 7기(false — 아치 안 밟는 면은 코일이 담당) · ★107(08.03) +3 = 나선 매스(walkable) + ②기둥·①보(false) · ★101(08.02) +5 = 판 고리·각뿔대 입술·바닥 슬래브·기단 고리 분기 + 빗면(false) · ★102 +3 = 감실 바닥·ⓑ계단(walkable) + 감실 천장/옆/뒤(false) · ★103 +2 = 슬롯 바닥(walkable) + 슬롯 옆·뒷벽(false) · ★104 +1 = 꺾인 상승 계단(walkable)
      ['Steles.jsx',         5,  0, 0, 0],
    ]
