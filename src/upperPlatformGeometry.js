...
//     막혔던 0° 문을 그대로 쓸 수 있다.
//
//  ★★132 갱신(2026.08.15) — **이 층의 근거는 소멸한 것이 아니라 연기됐다.**
//   현도가 ① 단일 통로를 1p4(315°) → 1p2(135°) 셸로 옮기면서 테라스 레벨 0° 대역 점유가 **일시적으로 0**이 됐다
//   (실측: 옛 배정 침범 11.2 → 현행 0). 그 상태라면 테라스 y127에서 바로 나가도 직선 **30.46°**로 상한 35° 안이다.
//   ⚠그러나 **0° 문의 임자는 여전히 셸 k0 = 1p4**이고, 그 셸은 아직 통로가 안 지어졌을 뿐이다
//   (현도: "드럼행 길 부근에 놓일 통로는 따로 그에 맞는 아이디어가 있다"). ③④가 배정되는 순간 0°는 다시 막힌다.
//   → **현도 확정 ⓒ(2026.08.15) = 새 층 유지 + 드럼행도 새 층에서.** 층 분리가 그 충돌을 미리 피하는 장치다.
//   대가 = 강하가 18.70 → 25.00으로 늘어 직선 **38.17°**(상한 35° 초과) — 수평 **3.90**을 더 벌어야 한다.
//   이 빚은 선언된 채로 남는다(경로 = 현도 그림 대기). 인과 전체는 `check_rooms` **★132절**이 잠근다.
//
