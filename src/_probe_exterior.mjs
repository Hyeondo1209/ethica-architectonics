...
function render(eye, yaw, pitch, out) {
  ...
  for (const t of tris) {
    ...
    for (let y = y0; y <= y1; y++) for (let x = x0; x <= x1; x++) {
      ...
      //  ⛔★도구 빚 ⑥ 수리(2026.08.22): 구판은 **NDC z를 화면공간 무게중심으로 아핀 보간**했다.
      //   화면공간에서 선형인 양은 깊이가 아니라 **1/w**다(원근 나눗셈의 정의). 아핀 보간은
      //   AM–HM 부등식에 의해 **항상 실제보다 멀게** 찍으며, 오차는 삼각형 안의 깊이비에 비례한다
      //   (깊이비 2 → 0.17 · 10 → 3.68 · 격리 실증 `_probe_affinez`). ⇒ 크고 비스듬한 삼각형(지붕판)이
      //   작고 조밀한 삼각형(볼트 웹)에게 깊이 시험을 **져서** ★148 "볼트 노출" 거짓 판정이 났다.
      //   1/w는 화면공간에서 정확히 선형이므로 아래가 **근사가 아니라 정확**하다. w > 0은 위 컬링이 보장.
      const iw = l0 / P[0].w + l1 / P[1].w + l2 / P[2].w
      const z = 1 / iw                                    // 뷰공간 깊이(작을수록 가깝다 — 구 NDC z와 대소 방향 동일)
      const i = y * W + x
      if (z >= zb[i]) continue
      zb[i] = z
